...
def benchmark_numba_optimization(json_path, iterations=20000, runs=10):
    """Benchmark Numba strategy with optimization iterations"""
    try:
...
def benchmark_cuda_optimization(json_path, iterations=20000, runs=10):
    """Benchmark CUDA strategy with optimization iterations"""
    try:
...
def run_optimization_benchmark():
    ...
    instances = [
        ...
        ('150-nodes', 'live-2025-example-instances/150-nodes.json', 150),
        ('225-nodes', 'live-2025-example-instances/225-nodes.json', 225),
    ]
    ...
    print(f"Iterations: {iterations:,}, Runs per test: 10")
    print("="*80)
    ...
    for name, path, nodes in instances:
        ...
        numba_result = benchmark_numba_optimization(path, iterations=iterations)
        if numba_result:
        ...
        cuda_result = benchmark_cuda_optimization(path, iterations=iterations)
        if cuda_result:
...
def plot_optimization_results(node_counts, instance_names, results):
    ...
    ax1.set_title('Optimization Time: CUDA vs Numba (20,000 iterations)\n(Lower is Better)', 
